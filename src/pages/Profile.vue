<template>
  <div class="min-w-80 mx-auto mt-8">
    ...
    <div v-else>
      <div class="space-y-3">
        <div>
          <label class="block text-xl font-medium text-gray-50">ФИО:</label>
          <p class="mt-1 p-2 w-full border-gray-300 rounded-md bg-gray-100">{{ fullName || 'Не заполнено' }}</p>
        </div>
        <div>
          <label class="block text-xl font-medium text-gray-50">Дата рождения:</label>
          <p class="mt-1 p-2 w-full border-gray-300 rounded-md bg-gray-100">{{ birthdate || 'Не заполнено' }}</p>
        </div>
        <div>
          <label class="block text-xl font-medium text-gray-50">Номер телефона:</label>
