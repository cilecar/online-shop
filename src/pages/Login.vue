  ...
  <div class="flex justify-center items-center h-screen">
    <div class="w-full max-w-md p-6 bg-white rounded-lg shadow-md">
      ...
      <form @submit.prevent="login" class="space-y-3">
        ...
        <button @click="goToReg" class="block w-full py-2 px-4 bg-gray-300 text-gray-800 rounded-md hover:bg-gray-400 mt-4">
  ...
  methods: {
    async login() {
      try {
        ...
        if (!response.ok) {
          throw new Error(data.error || 'Пользователь не найден или данные для входа неверные');
        }
